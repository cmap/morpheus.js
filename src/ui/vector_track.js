...
morpheus.VectorTrack.prototype = {
	...
	prePaint : function(clip, context) {
		// draw selection, highlighted, hover
		var project = this.project;
		var positions = this.positions;
		var height = this.getUnscaledHeight();
		var width = this.getUnscaledWidth();
	...
	draw : function(clip, context) {
		this._draw(clip, context, this.isColumns ? this.getUnscaledHeight()
				: this.getUnscaledWidth());
	},
	print : function(clip, context) {
		this._draw(clip, context, this.isColumns ? clip.height : clip.width);
	},
	_draw : function(clip, context, availableSpace) {
		var _this = this;
		var fullAvailableSpace = availableSpace;
		var positions = this.positions;
		var setup = this._setup(context, clip);
		var start = setup.start;
		var end = setup.end;
	...
	showPopup : function(e, isHeader) {
		...
		var DISPLAY_STACKED_BAR = 'Stacked';
	...
	renderColor : function(context, vector, start, end, clip, offset,
		...
		var canvasSize = isColumns ? this.getUnscaledHeight() : this
				.getUnscaledWidth();
		if (settings.colorBarSize > canvasSize) {
			settings.colorBarSize = canvasSize >= 5 ? (canvasSize - 1)
					: canvasSize;
		}
		var getColor;
		if (!continuous) {
		...
		for (var i = start; i < end; i++) {
			...
			if (isColumns) {
				context.beginPath();
				context.rect(position, offset - settings.colorBarSize, size,
						settings.colorBarSize);
				context.fill();
			} else {
				context.beginPath();
				context.rect(offset, position, settings.colorBarSize, size);
				context.fill();
			}
		}
	},
	renderShape : function(context, vector, start, end, clip, offset) {
		...
		for (var i = start; i < end; i++) {
			...
			var minSize = Math.min(settings.colorBarSize, itemSize);
			var size2 = minSize / 2;
	...
	renderText : function(context, vector, isColor, start, end, clip, offset,
		...
		if (isColumns) {
			context.translate(clip.x, clip.y); // reset transform
		}
